...
- **Dining** — all 13 reservations with tier ratings, times, and locations
- **Rider Swap** — height minimums by park and which rides Casper (40") can ride
...
Edit `index.html` directly, or re-run `generate_site.py` (the generator that built this page) and replace `index.html`. The booking checklist state lives in your browser's local storage, so it persists across visits on the same device.
